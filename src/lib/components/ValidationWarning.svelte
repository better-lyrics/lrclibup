	...
	>
		...
		{#if validationResult.hasELRC}
			<div class="text-xs text-red-700 bg-red-100/50 rounded p-2 mb-3">
				...
				<p class="leading-relaxed">
					Your lyrics contain ELRC word-level timestamps like <code class="bg-red-200 px-1 rounded">&lt;00:12.34&gt;word</code>.
					LRCLIB does not officially support ELRC format. Use the auto-fix button to strip word timestamps while keeping line timestamps.
				</p>
			</div>
		{/if}
